...
final class Shell
{
    ...
    public static function setLogFile($path,$prefix='')
    {
        if ($prefix)
        {
            $path=rtrim($path,DIRECTORY_SEPARATOR).DIRECTORY_SEPARATOR.$prefix;
        }


        self::$_logFilePath=$path;

        if (!is_dir(self::$_logFilePath))
        {
            mkdir(self::$_logFilePath,0777,true);
        }
    ...
    /**
     * In max Execution Time in seconds
     *
     * @param null $seconds
     * @return int
     */
    public static function maxExecutionTime($seconds=null)
    {

        if ($seconds)
        {
            set_time_limit($seconds);
            self::$_maxTimeMins=$seconds/60;
        }
    ...
    {
        self::msg($msg,[Shell::dark_gray,Shell::italic]);
    }
    static public function warning($msg)
    {
        self::msg($msg,[Shell::yellow]);
    }
    static public function info($msg)
    {
        self::msg($msg,[Shell::cyan]);
    }
    static public function error($msg,$sendAlert=true)
    {
//        fputs(STDERR,trim($msg)."\n");
        self::msg($msg,[Shell::white,Shell::bg_red,Shell::bold]);
    ...
    private static function checkShell()
    {
        ...
        if ($is==false)
        {
            self::debug("!Process not exist,not find by pid!\n");
            unlink($f);
            self::stopShell();
            return false;
        }
        ...
        // -----------// -----------
        $diff=(time()-$m)/60;
        if (self::$_maxTimeMins>0)
        {
            if ($diff>self::$_maxTimeMins)
            {
                self::warning("! Long Process PID : $pid, Running times: $diff seconds , try kill ....");
    ...
    static private function isICanRun()
    {
        ...
        if ( Shell::checkShell() )
        {
            ...
            if (self::get('wait'))
            {
                self::info("Can`t run pid exists : ".self::getPidFileName()." , try wait.");
                for($f=0;$f<500;$f++)
                {
                    sleep(1);
                    if (!Shell::checkShell() )
