<?php
namespace Shell;
// clone from github repo : https://github.com/kevinlebrun/colors.php/tree/master/src/Colors
class Color
{
    ...
    protected function stylize($style, $text)
    {
        ...
        throw new ShellException("Invalid style $style");
    }
    ...
    public function setUserStyles(array $userStyles)
    {
        foreach ($userStyles as $name => $styles) {
            if (!$this->isAValidStyleName($name)) {
                throw new ShellException("$name is not a valid style name");
            }

            if (in_array($name, (array) $styles)) {
                throw new ShellException('User style cannot reference itself.');
            }
        }
